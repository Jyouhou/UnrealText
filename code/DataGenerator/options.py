...
parser.add_argument('--UnrealProjectName', type=str, default='Demo_RealisticRend', help='') # TODO set later.. 
data_gen_opt_, _ = parser.parse_known_args()
data_gen_opt_.FontSize = list(map(int, data_gen_opt_.FontSize.split('/')))
data_gen_opt_.camera_anchor_filepath = osp.join(data_gen_opt_.camera_anchor_folder, f'{data_gen_opt_.scene}.txt')
data_gen_opt = dict()
